...
v1.0 완료 스냅샷은 [docs/README_v1_0.md](docs/README_v1_0.md)에 보관한다.
...
3. 버전이 완료되면 현재 `README.md`를 `docs/README_vX_Y.md`로 복사해 보관한다.
...
v1.1의 핵심 후보는 `/workspace`에 넣기 전의 생각을 자유롭게 모으는 공간이다.

현재 v1.0은 `Goal -> Plan -> Task` 구조가 잘 정리된 상태에서 사용하기 좋다. v1.1에서는 아직 구조화되지 않은 아이디어, 할 일, 메모를 빠르게 적고, 필요할 때 `/workspace`의 Goal / Plan / Task로 전환하는 흐름을 검토한다.
...
- [x] v1.0 완료 상태를 `docs/README_v1_0.md`로 보관
- [x] 루트 `README.md`를 v1.1 진행판으로 전환
- [x] 문서 관리 워크플로우 정리
- [ ] v1.1 완료 시 `docs/README_v1_1.md` 생성
