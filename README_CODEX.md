...
아직 mock 또는 미연결 단계인 것:

```txt
/whativedone, /timeline 데이터는 아직 Supabase에 연결되지 않았다.
Plan 카테고리 자체의 추가 / 수정 / 삭제는 아직 없다.
Dashboard TODO 클릭 시 /workspace 선택 상태 복원은 아직 연결되지 않았다.
...
각 항목에는 Task 제목, Goal / Plan breadcrumb, 상태, 날짜 범위, workspace 링크를 표시한다.
Goal / Plan 항목에는 하위 Task 기준으로 계산될 진행률을 표현할 수 있도록 progress bar를 표시한다.
이틀 이상 이어지는 Task는 RangeBar와 Range Tasks 패널에서 별도로 눈에 띄게 표시한다.
