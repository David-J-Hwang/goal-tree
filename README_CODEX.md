...
```txt
1. 새 버전 개발 시작
-> README.md를 해당 버전의 목표 / 체크리스트로 정리한다.
...
docs/README_v1_0_1.md
-> v1.0.1 완료 상태

docs/README_v1_1_0.md
-> v1.1.0 완료 상태

docs/README_v1_1_1.md
-> v1.1.1 완료 상태

docs/README_v1_1_2.md
-> v1.1.2 완료 상태
```
...
실제 개발 상태는 항상 파일 구조와 코드에서 최종 확인해야 한다.
...
2026-06-27 기준 v1.1.1은 완료되어 `docs/README_v1_1_1.md`에 스냅샷으로 보관했다.

2026-06-30 기준 v1.1.2는 완료되어 `docs/README_v1_1_2.md`에 스냅샷으로 보관했다.

현재 루트 `README.md`는 v1.2.0 진행판이다. v1.1.2 완료 상태는 `docs/README_v1_1_2.md`에서 확인한다.
...
Settings 모달의 Plan Categories는 `dnd-kit`으로 드래그 정렬한다. `/workspace`와 같은 `sortableStackModifiers`를 사용해 세로 스택 안에서만 부드럽게 움직이도록 했고, 순서는 `plan_categories.sort_order`에 저장한다. 이 컬럼은 `supabase/migrations/003_add_plan_category_sort_order.sql`에서 추가한다.
...
/whativedone Goal / Plan Contribution 실제 Node 상태와 Completion Log 현재 선택 기간 기준 계산
...
/timeline 날짜 범위 progress bar를 텍스트 표시로 교체
/timeline Done 항목 progress bar 제거
/timeline Upcoming 항목 progress bar 표시
/timeline 2일 이상 항목도 일반 카드 스타일 유지
...
Contribution 패널은 현재 선택된 Day / Week / Month / Year 기간에 완료된 Task만 기준으로 집계한다.
현재는 Supabase nodes 데이터를 읽어서 status=done이고 actualEndDate가 현재 선택 기간에 들어오는 Task를 표시한다.
```
...
날짜 범위는 progress bar가 아니라 `2026.06.10 - 2026.06.17, 8 days` 같은 텍스트로 표시한다.
Done 모드 카드에는 progress bar를 표시하지 않는다.
Upcoming 모드 카드에는 진행률 progress bar를 표시한다.
이틀 이상 이어지는 항목도 일반 카드 스타일을 유지하며, Range Tasks 패널에서 별도로 목록화한다.
...
v1.0.1 이후 첫 후속 기능은 자유 아이디어 수집 페이지다.
...
v1.1.1은 v1.1.0 이후의 향후 개발 후보를 다시 분류하는 단계로 시작했다. 현재 루트 `README.md`가 v1.1.1 진행판이다.

2026-06-27 기준 결정:

```txt
Google 로그인은 v1.1.1에서 제외하고 이후 업데이트 후보로 둔다.
비밀번호 재설정 / 수정 페이지도 v1.1.1에서 제외하고 이후 업데이트 후보로 둔다.
카테고리별 Plan 필터링은 v1.1.1 이후 업데이트 후보로 둔다.
미완료 TODO 자동 이월 설정은 현재 사용성상 필요하지 않아 보류 / 제외한다.
어제 미완료 TODO 가져오기 같은 수동 이월 기능도 현재는 필요하지 않아 보류 / 제외한다.
추가 페이지 / 확장 UX 항목은 v1.1.1에서 제외하고 이후 업데이트 후보로 둔다.
외부 연동 / 고급 기능 항목은 v1.1.1에서 제외하고 이후 업데이트 후보로 둔다.
```

v1.1.1 이후 버전 후보로 넘긴 작은 기능:
...
/inbox 카드 목록 컴포넌트는 모바일부터 고정 높이와 내부 스크롤을 사용하고, 카드에는 제목만 표시하며 텍스트/상태 뱃지를 세로 중앙 정렬
/workspace Goals/Plans/Tasks 컴포넌트도 모바일부터 고정 높이와 내부 스크롤을 사용
/inbox Details에서 수정사항이 있을 때 Delete card를 숨기고 Discard/Save 버튼만 한 줄로 배치
모바일 환경 전체 페이지의 주요 컴포넌트 높이 고정 여부 확인
```

`/inbox` 카드는 아직 Workspace node가 아니므로 완료 상태를 바꾸더라도 `/whativedone`, `/timeline` 기록에는 직접 반영되지 않는다. 기록 페이지에 남기려면 Inbox 카드를 `/workspace`의 Goal / Plan / Task로 전환한 뒤 해당 Workspace 카드의 상태가 완료되어야 한다.

v1.1.1은 데스크탑 / 패드 / 모바일 환경에서 주요 UI/UX 확인까지 마친 뒤 `docs/README_v1_1_1.md`로 스냅샷을 남겼다.

## 26. v1.1.2: 날짜 / 기록 표시 안정화

v1.1.2는 날짜 / 시간 기준, Timeline 기간 표시, What I've Done Contribution 집계 기준을 다듬은 마이너 업데이트다.

완료 범위:

```txt
서버 시간대 영향으로 오늘 날짜가 어긋나지 않도록 앱 날짜 기준 정리
/dashboard 오늘 날짜와 Daily TODO 조회 날짜 안정화
/timeline 날짜 범위 progress bar를 텍스트로 교체
/timeline 날짜 범위 텍스트는 yyyy.MM.dd - yyyy.MM.dd, N days 형식
/timeline 2일 이상 카드도 일반 카드 스타일 유지
/timeline Done 탭에서는 progress bar 제거
/timeline Upcoming 탭에서는 progress bar 표시
/whativedone Goal / Plan Contribution을 Completion Log 현재 탭 기간 기준으로 집계
```

v1.1.2 완료 상태는 `docs/README_v1_1_2.md`에 스냅샷으로 보관했다.

## 27. v1.2.0: 다음 메이저 업데이트

현재 루트 `README.md`는 v1.2.0 진행판이다.

v1.2.0은 v1.1.x의 마이너 개선 흐름을 마무리한 뒤 진행하는 다음 메이저 업데이트다. 아직 목표와 범위는 확정 전이며, 루트 `README.md`의 후보 목록에서 이번 메이저 버전에 포함할 작업을 먼저 고른다.
